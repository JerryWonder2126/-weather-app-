...
<img src='./workflow.jpg' alt='Workflow of the app on pageload' style='display:block;' />
...
- Set an environment variable with variable name of ``OPEN_WEATHER_API_KEY``, assign your access key to this. Also create another variable ``PORT`` to specify the port on which the localhost will run. Since we are running this locally, we can set this up easily with ``dotenv``.
